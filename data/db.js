...
let DB_PEDIDOS = [
  ...
  },
  {
    ...
    estado: "pendiente",
    detalle: [
      { producto_id: 1, cantidad: 1, precio_unitario: 28000, subtotal: 28000 },
      { producto_id: 8, cantidad: 2, precio_unitario: 8000, subtotal: 16000 },
    ],
  },
];
...
function dbGetEmpleados() {
  return DB_USUARIOS.map((u) => ({
    id: u.id,
    firstName: u.nombre.split(" ")[0],
    lastName: u.nombre.split(" ").slice(1).join(" "),
    cedula: u.cedula,
    role: u.rol,
    shift: u.id % 2 === 0 ? "morning" : "afternoon",
    status: "active",
  }));
}

function dbCrearEmpleado({ firstName, lastName, cedula, role, shift }) {
  const nuevo = {
  ...
  DB_USUARIOS.push(nuevo);
  return nuevo;
}

function dbActualizarEmpleado(
  id,
  { firstName, lastName, cedula, role, shift },
) {
  const u = DB_USUARIOS.find((u) => u.id === id);
  if (!u) return;
  u.nombre = firstName + " " + lastName;
  u.cedula = cedula;
  u.rol = role;
}

function dbEliminarEmpleado(id) {
  const idx = DB_USUARIOS.findIndex((u) => u.id === id);
  if (idx !== -1) DB_USUARIOS.splice(idx, 1);
}

function dbCedulaDuplicada(cedula, excludeId = null) {
  return DB_USUARIOS.some((u) => u.cedula === cedula && u.id !== excludeId);
}
