...
// SIDEBAR
// ================================
function cargarSidebar(user, page) {
  const container = document.getElementById("admin-sidebar");
  if (!container) return;

  fetch("../../layout/sidebar-admin.html")
    .then((r) => r.text())
    .then((html) => {
      ...
      // Marcar activo
      const links = container.querySelectorAll(".sidebar-nav-item[data-page]");
      links.forEach((link) => {
...
// NAVBAR
// ================================
function poblarNavbar(user) {
  const nombre = user.nombre || "Admin";
  const inicial = nombre.charAt(0).toUpperCase();

  const nameEl = document.getElementById("nav-user-name");
  const avatarEl = document.getElementById("nav-user-avatar");
  const perfilNombre = document.getElementById("perfil-nombre-grande");
  const perfilAvatar = document.getElementById("perfil-avatar-grande");
  ...
  if (perfilCedula) perfilCedula.textContent = user.cedula || "—";
}
...
function togglePerfilPanel() {
  ...
  const overlay = document.getElementById("panel-overlay");
  const notif = document.getElementById("panel-notif");
...
function initDashboard() {
  ...
  if (el("stat-mesas"))
    el("stat-mesas").textContent = ocupadas + " / " + totalM;
  ...
  if (pedidos.length > 0) {
    ...
    if (el("stat-ventas"))
      el("stat-ventas").textContent = formatPrecio(totalVentas);
...
function renderEmployees() {
  ...
  if (countEl)
    countEl.textContent = employees.length + " empleados registrados";
}
...
function updateStats() {
  ...
  if (s("stat-active"))
    s("stat-active").textContent = e.filter(
      (x) => x.status === "active",
    ).length;
  if (s("stat-morning"))
    s("stat-morning").textContent = e.filter(
      (x) => x.shift === "morning",
    ).length;
  if (s("stat-afternoon"))
    s("stat-afternoon").textContent = e.filter(
      (x) => x.shift === "afternoon",
    ).length;
}
...
function saveEmployee() {
  ...
  if (editingId) {
    dbActualizarEmpleado(editingId, {
      firstName,
      lastName,
      cedula,
      role,
...
function initReportes() {
  ...
  if (rangoEl)
    rangoEl.textContent = "Semana " + fmt(lunes) + " – " + fmt(domingo);
  ...
  const el = (id) => document.getElementById(id);
  if (el("r-ventas-total"))
    el("r-ventas-total").textContent = formatPrecio(totalVentas);
  if (el("r-ticket")) el("r-ticket").textContent = formatPrecio(ticket);
  if (el("r-ordenes")) el("r-ordenes").textContent = totalOrdenes;
  if (el("r-ordenes-day"))
    el("r-ordenes-day").textContent =
      Math.round(totalOrdenes / 7) + " órdenes/día";
  if (el("r-propinas")) el("r-propinas").textContent = formatPrecio(propinas);
  if (el("r-ventas-change"))
    el("r-ventas-change").textContent = "+18% vs semana anterior";
  if (el("r-ticket-change"))
    el("r-ticket-change").textContent = "+13% vs semana anterior";
